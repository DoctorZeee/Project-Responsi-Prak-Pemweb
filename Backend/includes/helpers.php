...
// ------------------------------------------------------------
// Buat slug yang unique di tabel items
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// Upload gambar item — returns filename atau null
// ------------------------------------------------------------
function uploadItemImage(array $file, string $uploadDir): ?string {
    ...
    if ($file['size'] > 2 * 1024 * 1024) return null; // max 2 MB
...
// ------------------------------------------------------------
// JSON response helper (untuk API endpoints)
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// Hitung rata-rata rating & update ke tabel items
// ------------------------------------------------------------
function recalcAvgRating(PDO $db, int $itemId): void {
    ...
    $db->prepare('UPDATE items SET avg_rating = ? WHERE id = ?')
       ->execute([$avg, $itemId]);
}
